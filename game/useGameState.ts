...
import { CHARACTERS, CANVAS_WIDTH, CANVAS_HEIGHT, DEFAULT_SETTINGS, GRID_COLS, GRID_ROWS, STAMINA_CONFIG, PHYSICS } from '../constants';
...
export function useGameState() {
  ...
  const stageReadyTimeRef = useRef(0); // Time when exit opened
  ...
  const cameraRef = useRef<CameraState>({
      ...
      x: 0,
      y: 0,
      zoom: 1.25, // Updated default zoom
      tilt: 0,
      isLocked: false,
      scrollSpeed: 0,
      targetMode: null,
  ...
  // SHARED INTENT QUEUE FOR CAMERA (Fixes Hook Isolation)
  ...
  // ── TRAIT RECALCULATION ON LEVEL UP ──
  useEffect(() => {
      // Whenever UI Level changes (which tracks levelRef), update scaling traits
      if (selectedChar) {
          traitsRef.current = resolveTraits(selectedChar, uiLevel);
      }
  }, [uiLevel, selectedChar]);
  ...
  const resetGame = useCallback((charProfile: CharacterProfile) => {
    ...
    floor.addFloorVolume({
        ...
        startX: -1000,
        endX: 100000,
        topY: viewport.rows - 2 // DYNAMIC FLOOR PLACEMENT
    });
    ...
    // Initialize Camera to player start position to avoid jump
  ...
  }, [syncUiStats, floor.clearFloors, floor.addFloorVolume, viewport]);
  ...
  return {
    ...
    viewport, setViewport, // EXPORTED HERE
    uiScore, setUiScore,
    uiXp, setUiXp,
    ...
    invulnerabilityTimeRef, audioEventsRef, lastDamageTimeRef, deathTimerRef,
    transitionStartTimeRef, pendingStatusRef, stageArmedRef, stageReadyRef, stageReadyTimeRef,
    ...
    lastPowerUpStateRef,
    transitionStateRef,
    cameraRef, cameraIntentsRef, updateCamera, requestCameraSwitch,
    physicsRef, jumpIntentRef, 
    floor, 
    resetGame, openSettings, closeSettings, togglePause,
    unlockedCosmetics, purchasedCosmetics, neonFragments, sessionNewUnlocks, setSessionNewUnlocks, toastQueue, seenCosmetics, hasNewCosmetics,
